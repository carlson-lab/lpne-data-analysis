function formatWindowsSeparateEventDirectory(saveFile, useIntervals, centeredWindows, projectFolder, chanFile, fs, ...
windowLength,centerFolder)
% formatWindows
...
for k = 1:nSessions
    %thisFile = dataList(k);
    thisFile = centerList(k);
    if thisFile.isdir, continue, end

    % clear channel data from last file and load it from the next file
    clear('-regexp','_\d\d')
    centername = thisFile.name;
    %filename = thisFile.name;
    c = strsplit(centername,'_');
    filename = [c{1} '_' c{2} '_' c{3} '_LFP.mat'];
    if strcmp(filename,'Mouse04394_080420_RRow_LFP.mat')
        filename2 = 'Mouse04394_080420_RRow_LFP_Rebuild.mat';
        load(dataPath)
    elseif strcmp(filename,'Mouse04394_081820_RRow_LFP.mat')
        filename2 = 'Mouse04394_081820_RRow_LFP_Rebuild.mat';
        dataPath = [dataFolder filename2];
        load(dataPath)
    else
    ...
    centerFile = [centerFolder regexprep(filename, 'LFP.mat', 'OffEvents.mat')];
    ...
    for c = 1:C
        ...
        elseif centeredWindows
            ...
            for i = 1:nWindows
                ...
                if (timestamps(i)+halfWindow)<length(thisChannel)
                    ...
                    usableData(1:pointsPerWindow, i) = -100000; % Assume will be killed later
                end
            end
