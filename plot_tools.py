...
def factor_gridplot(diags, offdiags1, offdiags2, areaList=None, freqs=1, labels=('',)):
    """
    Plots a factor in 'upper triangular' grid format

    :param diags: contains features that should be plotted in subfigures
        along the diagonal (e.g. PSD)
    :param offdiags1: contains first set of features to be plotted in
        offdiagonal subfigures, should have same scale as diag features
    :param offdiags2: contains second set of features to be plotted in
        offdiagonal subfigures, can have different scaling than diag features
    :param areaList: list of areas that features are generated from
    :param freqs: list of frequencies or scalar frequency sampling
        spacing (Hz)
    :param labels: tuple of 2 y-axis text labels. First label is applied
        to diags and offdiags1. Second label applies to offdiags2
    """
    YLIM_SCALE = 1.15
    R_LABEL_SIZE = 12
    R_LABEL_X_OFF = -27
...
def factor_features(feature_str, feature_labels, factor):
    """
    Return the indicated subset of factor features

    :param feature_str: string indicating type of features to extract
    :param feature_labels: list of labels indicating the features
        represented in 'factor'. These are generated by 'saveFeatures.m'
        and contained in the labels loaded in by 'load_data'
    :param factor: compnents vector, or loadings, from a single factor
        of a linear factor model
    :returns: a specified subset of the components vector
    """
    f_list = [re.findall(feature_str, fl) for fl in feature_labels]
